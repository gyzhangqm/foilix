...
r"""pyswarm example from https://github.com/tisimst/pyswarm"""

import numpy as np
# from pyswarm import pso
from foilix.optimization.pyswarm import pso

print("*"*65)
print('Example minimization of 4th-order banana function (no constraints)')


def my_function_to_optimize(x, *args):
    r"""The function to optimize
    ...
    x1 = x[0]
    x2 = x[1]
    return x1**4 - 2*x2*x1**2 + x2**2 + x1**2 - 2*x1 + 5 + a

lb = [-3, -1]  # lower bound
ub = [2, 6]  # upper bounds

xopt1, fopt1 = pso(my_function_to_optimize, lb, ub, args=(1,))
...
xopt2, fopt2 = pso(my_function_to_optimize, lb, ub, f_ieqcons=mycon, args=(1,))
