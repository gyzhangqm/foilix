...
r"""from https://gist.github.com/d1manson

Summary
-------
Fork of pyswarm (particle swarm optimization in python)
This version has been vectorised/optimised more than the original
...
def pso(func, lb, ub, ieqcons=None, f_ieqcons=None, args=(), kwargs=None,
    ...
    if f_ieqcons is None:
        if not len(ieqcons):
            ...
            is_feasible = lambda x: np.all((y(x, *args, **kwargs) >= 0
                                            for y in ieqcons))
    else:
        if debug:
    ...
    for it in range(1, maxiter + 1):  # GF : Python 3 compatibility
        ...
        # update particles individual best values
        is_best = fx < fp
        if np.any(is_best):
            is_best[is_best] &= np.array([is_feasible(xx)
                                          for xx in x[is_best, :]])
        ...
        if fx[fx_min_ind] < fg:
            ...
            if np.abs(fg - fg_old) <= minfunc:
                if info:
                    print('Stopping search: Swarm best objective change '
                          'less than {:}'.format(minfunc))
                return g, fg
            elif np.sqrt(np.sum((g-g_old)**2)) <= minstep:
                if info:
                    print('Stopping search: Swarm best position change '
                          'less than {:}'.format(minstep))
                return g, fg
    ...
    if info:
        print('Stopping search: maximum iterations '
              'reached --> {:}'.format(maxiter))
